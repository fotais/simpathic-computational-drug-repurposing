...
def create_pam_matrices(
    ...
    wanted_nodes_start: list = [],
    wanted_nodes_end: list = [],
    ...
    Returns:
        tuple[csr_array, list[csr_array], dict, dict, bool]: The first argument is the lossless 1-hop PAM with products.
    ...
    time_s = time.time()
    unique_rels = sorted(list(df_train["relation"].unique()))
    unique_nodes = sorted(
        set(df_train["head"].astype(str).values.tolist() + df_train["tail"].astype(str).values.tolist())  # type: ignore
    )
    ...
    df_train["rel_mapped"] = df_train["relation"].map(rel2id)
    ...
    if len(wanted_nodes_start) > 0:
        ...
        pam_1hop_lossy = pam_1hop_lossy[wanted_ids]
    ...
    print(f"Sparsity 1-hop: {sparsity:.2f} %)")
